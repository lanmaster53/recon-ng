...
class Recon(framework.Framework):
    ...
    def __init__(self, mode):
        ...
        self._init_home()
        self._load_modules()
        if self._mode == Mode.CONSOLE: self.show_banner()
    ...
    def do_load(self, params):
        ...
        if self._mode == Mode.CLI: return y
        # begin a command loop
        try: y.cmdloop()
        except KeyboardInterrupt:
            print('')
        if y._exit == 1: return True
    do_use = do_load
    ...
    def help_snapshots(self):
        ...
        print('')
        print('Usage: workspaces [list|take|load|delete]')
        print('')
